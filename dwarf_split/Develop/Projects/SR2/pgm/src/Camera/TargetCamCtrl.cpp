/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Camera\TargetCamCtrl.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
float m_tof32LandSurfaceRate; // size: 0x4, address: 0x0
float m_tof32KickerFovy; // size: 0x4, address: 0x0
static float tou32IpoFrame; // size: 0x4, address: 0x669990
struct /* @anon0 */ {
    // total size: 0x2C
...
struct /* @anon3 */ {
    // total size: 0x30
} __vt__16clsDebugLineTask; // size: 0x30, address: 0x0
struct /* @anon2 */ {
    // total size: 0x30
...
class clsDebugLineTask : public clsTask {
    // total size: 0x70
public:
    class NNS_VECTOR m_asVec[2]; // offset 0x54, size 0x18
    unsigned int m_u32Color; // offset 0x6C, size 0x4
};
...
class stcSpeedMixParam {
    // total size: 0x30
public:
    float f32HeightFromTgt; // offset 0x0, size 0x4
    float f32Dist; // offset 0x4, size 0x4
    float f32Fovy; // offset 0x8, size 0x4
    float f32IpoRot; // offset 0xC, size 0x4
    float f32IpoDir; // offset 0x10, size 0x4
    float f32IpoTra; // offset 0x14, size 0x4
    float f32IpoUp; // offset 0x18, size 0x4
    class NNS_VECTORFAST sEyeTgtVF; // offset 0x20, size 0x10
};
...
// Range: 0x2B3350 -> 0x2B33A4
void resetData(class clsTargetCam * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x2B3350 -> 0x2B33A4
    }
...
// Range: 0x2B33B0 -> 0x2B34B4
void getParams(class clsTargetCam * this /* r17 */, void * vpParams /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x2B33B0 -> 0x2B34B4
        class stcTargetCam1 * psTgtParam; // r2
    }
}


// Range: 0x2B34C0 -> 0x2B34D8
void startMixing(class clsTargetCam * this /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x2B34C0 -> 0x2B34D8
    }
...
// Range: 0x2B34E0 -> 0x2B3744
void setParams(class clsTargetCam * this /* r17 */, void * vpParams /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x2B34E0 -> 0x2B3744
        class stcTargetCam1 * psTgtParam; // r2
...
// Range: 0x2B3750 -> 0x2B4C3C
unsigned int execute(class clsTargetCam * this /* r17 */, class clsPfCamera * pcCamera /* r16 */, class clsPfCamera * opcBaseCamera /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x2B3750 -> 0x2B4C3C
        class NNS_VECTORFAST sCamPosWVF; // r29+0x350
        class NNS_VECTORFAST sEyeTgtWVF; // r29+0x340
        class NNS_VECTORFAST sDownLVF; // r29+0x330
        float sTmpMtx[4][4]; // r29+0x2F0
        class NNS_VECTORFAST sDistLVF; // r29+0x2E0
        class NNS_VECTORFAST sEyeTgtLVF; // r29+0x2D0
        class NNS_VECTORFAST sTgtUpVF; // r29+0x2C0
        class NNS_VECTORFAST sHeightLVF; // r29+0x2B0
        class NNS_VECTORFAST sCamXAxisLVF; // r29+0x2A0
        class NNS_VECTORFAST sCamZAxisLVF; // r29+0x290
        class NNS_VECTORFAST sCamUpLVF; // r29+0x280
        class NNS_VECTORFAST sBf_PosWVF; // r29+0x270
        class NNS_VECTORFAST sBf_TgtWVF; // r29+0x260
        class NNS_VECTORFAST sBf_UpLVF; // r29+0x250
        class NNS_VECTORFAST sAdjustPosVF; // r29+0x240
        class NNS_VECTORFAST sTargetPosVF; // r29+0x230
        class NNS_QUATERNION sQuat; // r29+0x220
        class NNS_QUATERNION sRotQuat; // r29+0x210
        class NNS_QUATERNION sDirQuat; // r29+0x200
        float f32Rad; // r20
        float f32Dir; // r21
        class stcSpeedMixParam sMixParam; // r29+0x1D0
        float f32NowRad; // r29+0x360
        unsigned char u8FovFlg; // r20
        float f32FrameRate; // r20
        float f32TmpY; // r29+0x360
        unsigned int u32Flag; // r19
        class stcCamFilter sTmpFilter; // r29+0x190
        class stcCamFilter * psFilter; // r18
        class NNS_VECTORFAST sTgtTmpVF; // r29+0x180
        class NNS_VECTORFAST sSysUpVF; // r29+0x170
        class NNS_VECTORFAST sPlayerMoveVF; // r29+0x160
        class NNS_VECTORFAST tgtvf; // r29+0x150
        class NNS_VECTORFAST upvf; // r29+0x140
        class NNS_VECTORFAST sPlToCamVF; // r29+0x130
        class NNS_VECTORFAST sDstPosVF; // r29+0x120
        class NNS_VECTORFAST sJustEyeLVF; // r29+0x110
        class NNS_VECTORFAST sOriginVF; // r29+0x100
        class NNS_VECTORFAST sNowEyeLVF; // r29+0xF0
        float tf32Limit; // @ 0x006245E0
    }
}


// Range: 0x2B4C40 -> 0x2B511C
void adjustMaxSpeed(class clsTargetCam * this /* r18 */, class stcSpeedMixParam * psSpeedMixParam /* r17 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x2B4C40 -> 0x2B511C
        class NNS_VECTORFAST sEyePosFromTgtVF; // r29+0xA0
        float f32FrameRate; // r22
        class stcCamFilter sTmpFilter; // r29+0x60
        class stcCamFilter * psFilter; // r16
        float f32Rate; // r21
        class NNS_VECTORFAST sTmpTgtVF; // r29+0x50
    }
}
