/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\2D\Score\ScoreMap2D.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
static class stcDrawParam oasStageParam1v[17]; // size: 0x1DC, address: 0x681C80
static unsigned char chk__oasStageParam1v_SizeCheck_[1]; // size: 0x1, address: 0x0
static class stcDrawParam oasStageParam2v[17]; // size: 0x1DC, address: 0x681E60
static unsigned char chk__oasStageParam2v_SizeCheck_[1]; // size: 0x1, address: 0x0
static class stcDrawParam oasStageParam4v[17]; // size: 0x1DC, address: 0x682040
static unsigned char chk__oasStageParam4v_SizeCheck_[1]; // size: 0x1, address: 0x0
static float toaf32BaseRot[10]; // size: 0x28, address: 0x0
static float toaf32DiffRot[10]; // size: 0x28, address: 0x0
class NNS_OBJECT * m_tpsMapObject; // size: 0x4, address: 0xB16390
static class stcRotGroundPath asRotPath[10]; // size: 0x50, address: 0x6359D0
class stcDrawParam tsDebugMap; // size: 0x1C, address: 0xB163A0
struct /* @anon0 */ {
    // total size: 0x20
} __vt__13clsScoreMap2D; // size: 0x20, address: 0x6C18B0
...
class stcRotGroundPath {
    // total size: 0x8
public:
    float f32Rate; // offset 0x0, size 0x4
    signed short s16x; // offset 0x4, size 0x2
    signed short s16y; // offset 0x6, size 0x2
};
class stcInitData {
    // total size: 0x30
public:
...
// Range: 0x3C41A0 -> 0x3C4268
void drawMapMini(signed short s16BackX /* r19 */, signed short s16BackY /* r16 */, unsigned char u8DrawAlpha /* r18 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3C41A0 -> 0x3C4268
        class stcTexUV sTexSrc; // @ 0x00635A20
    }
}


// Range: 0x3C4270 -> 0x3C4524
void drawMapBg(signed short s16BackX /* r21 */, signed short s16BackY /* r20 */, unsigned char u8DrawAlpha /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3C4270 -> 0x3C4524
        class stcPoint2D_s16 * psTextureRaceEtcSize; // r2
        signed int s32BackHalfWidth; // r2
        signed int s32BackHalfHeight; // r2
    }
}


// Range: 0x3C4530 -> 0x3C4D34
void draw(class clsScoreMap2D * this /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3C4530 -> 0x3C4D34
        class stcTexUV * psTexSrc; // r2
        class stcTexUV * psTexSrc_RaceIcon; // r2
        class stcPoint2D_s16 * psTextureRaceIconSize; // r2
        unsigned char u8DrawAlpha; // r2
        float sViewMtx44[4][4]; // r29+0x180
        float sClipMtx[4][4]; // r29+0x140
        class stcPoint2D_s16 sRetPoint2D; // r29+0x1CC
        float f32HalfWidth; // r29+0x1D0
        float f32HalfHeight; // r29+0x1D0
        unsigned char u8CharaID; // r3
        float f32HalfWidth; // r29+0x1D0
        float f32HalfHeight; // r29+0x1D0
        signed int s32n; // r20
        class clsPlayerTask * pcPlayer; // r19
        unsigned char u8CharaID; // r18
        unsigned char u8IconAlpha; // r22
        float sDrawMtx[4][4]; // r29+0x100
        float f32Scale; // r20
    }
}


// Range: 0x3C4DA0 -> 0x3C4F50
void clip2Screen(class clsScoreMap2D * this /* r19 */, class stcPoint2D_s16 & rsDstScreenO /* r18 */, signed int s32vI /* r2 */, float (& orsMtx44I)[4][4] /* r2 */, class NNS_VECTORFAST & orsSrcVecI /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3C4DA0 -> 0x3C4F50
        class NNS_VECTORFAST sClipPosI; // r29+0x50
    }
}


// Range: 0x3C4F50 -> 0x3C50FC
void execute(class clsScoreMap2D * this /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3C4F50 -> 0x3C50FC
        signed int s32AddRot; // r2
        signed int s32n; // r8
    }
}


// Range: 0x3C5100 -> 0x3C514C
void * __dt(class clsScoreMap2D * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3C5100 -> 0x3C514C
    }
...
// Range: 0x3C5150 -> 0x3C5464
void * __ct(class clsScoreMap2D * this /* r17 */, class clsScore2D_Task * pcParent /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3C5150 -> 0x3C5464
        signed int s32BackHalfWidth; // r2
