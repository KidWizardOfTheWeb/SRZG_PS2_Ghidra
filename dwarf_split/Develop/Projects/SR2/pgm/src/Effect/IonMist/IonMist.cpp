/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Effect\IonMist\IonMist.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
class _NNS_TEXLIST * m_psTex; // size: 0x4, address: 0xB15E38
unsigned char m_tu8DrawFlg; // size: 0x1, address: 0xB15E40
unsigned char m_tbSpFlg; // size: 0x1, address: 0xB15E48
class stcParam m_tsDebugFullParam; // size: 0x6C, address: 0xB15E50
class stcDebugParam m_tsDebugParam; // size: 0x8, address: 0xB15EC0
static class stcParam StageParam[5]; // size: 0x21C, address: 0x67EEF0
struct /* @anon0 */ {
    // total size: 0x30
} __vt__15clsIonMist_Task; // size: 0x30, address: 0x6C0060
struct /* @anon1 */ {
    // total size: 0x30
...
float of32GaugeZ; // size: 0x4, address: 0x66ADD8
...
class clsIonMist_Task : public clsTask {
    // total size: 0xE4
public:
    unsigned char m_u8StageNo; // offset 0x54, size 0x1
    class stcParam m_sFullParam; // offset 0x58, size 0x6C
    class stcTexUV m_sNTex; // offset 0xC4, size 0x10
    class stcTexUV m_sFTex; // offset 0xD4, size 0x10
};
class PotentialInfo {
    // total size: 0x1008
public:
...
class stcNearStatus {
    // total size: 0x30
public:
    class NNS_VECTOR sStartPos; // offset 0x0, size 0xC
    float f32StartAlpha; // offset 0xC, size 0x4
    class NNS_VECTOR sEndPos; // offset 0x10, size 0xC
    float f32EndAlpha; // offset 0x1C, size 0x4
    class stcTexArea sTexArea; // offset 0x20, size 0x10
};
class hkArray {
    // total size: 0xC
public:
...
public:
    class NNS_VECTOR sStartPos; // offset 0x0, size 0xC
    float f32ClipVec; // offset 0xC, size 0x4
    class NNS_VECTOR sEndPos; // offset 0x10, size 0xC
    float f32Alpha; // offset 0x1C, size 0x4
    class stcTexArea sTexArea; // offset 0x20, size 0x10
};
enum enmStatus {
    STATUS_NULL = 0,
...
public:
    ...
    float f32TuSpd; // offset 0x4, size 0x4
    float f32TvSpd; // offset 0x8, size 0x4
    float f32Size; // offset 0xC, size 0x4
};
...
public:
    unsigned int u32Flg; // offset 0x0, size 0x4
    unsigned char u8StageNo; // offset 0x4, size 0x1
    unsigned char u8PortalNum; // offset 0x5, size 0x1
    char ac8Portal[4]; // offset 0x6, size 0x4
    class stcNearStatus sNear; // offset 0xC, size 0x30
    class stcFarStatus sFar; // offset 0x3C, size 0x30
};
class NVS_SVROBJ {
    // total size: 0x30
public:
...
public:
    unsigned int u32Page; // offset 0x0, size 0x4
    unsigned char bUsed; // offset 0x4, size 0x1
};
class stcDrawPath {
    // total size: 0x2B
public:
...
// Range: 0x391240 -> 0x3917F4
void draw(class clsIonMist_Task * this /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x391240 -> 0x3917F4
        signed int s32ViewNo; // r20
        unsigned int u32DrawFlg; // r19
    }
}


// Range: 0x391800 -> 0x391A94
unsigned char calcFarAlpha(class clsIonMist_Task * this /* r21 */, unsigned int u32View /* r2 */, float * pf32OfsZ /* r20 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x391800 -> 0x391A94
        unsigned int u32PMFlg; // r2
        float f32Dist; // r20
        float f32DistMax; // r1
        float f32Alpha; // r1
        float f32AxisDir; // r3
        class NNS_VECTOR sPosDiff; // r29+0x90
        class NNS_VECTOR sWork; // r29+0x80
    }
}


// Range: 0x391AA0 -> 0x391DDC
unsigned char calcNearAlpha(class clsIonMist_Task * this /* r18 */, unsigned int u32View /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x391AA0 -> 0x391DDC
        unsigned char u8Alpha; // r2
        unsigned int u32PMFlg; // r2
        float f32AlphaDiff; // r20
        float f32Dist; // r3
        float f32DistMax; // r21
        float f32Alpha; // r1
        float f32CalcPos; // r2
        float f32DirPos; // r1
        class NNS_VECTOR sWork; // r29+0x50
    }
}


// Range: 0x391DE0 -> 0x392028
void execute(class clsIonMist_Task * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x391DE0 -> 0x392028
    }
...
// Range: 0x392030 -> 0x392384
void anmTexArea(class stcTexUV * psTex /* r18 */, class stcTexArea * psTexArea /* r17 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x392030 -> 0x392384
    }
...
// Range: 0x392390 -> 0x394624
signed int DebugInfo(signed int s32xI /* r18 */, signed int s32yI /* r17 */, signed int s32ActiveNoI /* r22 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x392390 -> 0x394624
        signed int s32SelectMenuItem; // r16
        signed int s32yBack; // r29+0xFC
        signed int s32n; // r21
        float f32SpeedI; // r20
        char * OnOffName[2]; // r29+0x198
        char * DrawName[4]; // r29+0x180
        char * DistName[6]; // r29+0x160
        char * DirName[5]; // r29+0x140
        char * CheckName[6]; // r29+0x120
        char * pac8FlgName[5]; // r29+0x100
        class stcParam * psFullParam; // r20
        class stcDebugParam * psDebugParam; // r2
        unsigned int u32DrawFlg; // r23
        unsigned int u32DistFlg; // r2
        unsigned int u32DirFlg; // r2
        unsigned int u32CheckFlg; // r2
        class stcNearStatus * psNear; // r2
        class stcFarStatus * psFar; // r2
        class stcTexArea * psTexArea; // r2
        unsigned int u32DrawFlgI; // r2
        unsigned int u32DistFlgI; // r2
        unsigned int u32DirFlgI; // r2
        unsigned int u32CheckFlgI; // r2
        signed int s32PortalNo; // r2
        class stcNearStatus * psNear; // r20
        class stcFarStatus * psFar; // r20
    }
}


// Range: 0x394630 -> 0x39499C
void setData(class clsIonMist_Task * this /* r16 */, unsigned char u8StageNo /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x394630 -> 0x39499C
        unsigned char bSet; // r8
        signed int s32n; // r5
    }
}


// Range: 0x3949A0 -> 0x394A40
void * __ct(class clsIonMist_Task * this /* r16 */, unsigned char u8StageNo /* r17 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3949A0 -> 0x394A40
    }
