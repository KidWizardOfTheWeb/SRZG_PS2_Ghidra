/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Effect\Weather.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
static class stcStatus oasStatus[6]; // size: 0x270, address: 0x6837E0
static class stcStatus oasStatusMulti[6]; // size: 0x270, address: 0x683A50
static class stcWaterDropStatus oasWaterDropStatus[2]; // size: 0x30, address: 0x683CC0
class stcDebugWeather sDebugInfo; // size: 0x70, address: 0x635CC0
static class NNS_VECTOR tsTestColPos; // size: 0xC, address: 0xB16D78
static class NNS_VECTOR tsTestColRot; // size: 0xC, address: 0xB16D88
static class NNS_VECTOR tsTestColScale; // size: 0xC, address: 0x635D30
static class NNS_VECTOR DefArea; // size: 0xC, address: 0x0
static class NNS_VECTOR osWindyVec[8]; // size: 0x60, address: 0x683CF0
void (* m_oasexeWeather[9])(class stcStatus *, class stcSeed *, float (*)[4][4]); // size: 0x24, address: 0x0
struct /* @anon1 */ {
    // total size: 0x30
} __vt__15clsWeather_Task; // size: 0x30, address: 0x6C2390
...
float tosUnitMatrix[4][4]; // size: 0x40, address: 0x684650
...
class stcDebugWeather {
    // total size: 0x70
public:
    unsigned char u8Page; // offset 0x0, size 0x1
    signed int s32PlayNum; // offset 0x4, size 0x4
    unsigned int u32Flg; // offset 0x8, size 0x4
    unsigned char u8Weather; // offset 0xC, size 0x1
    unsigned char u8Windy; // offset 0xD, size 0x1
    unsigned char u8WindyWay; // offset 0xE, size 0x1
    signed int s32Num; // offset 0x10, size 0x4
    unsigned char u8Rand; // offset 0x14, size 0x1
    float f32Height; // offset 0x18, size 0x4
    float f32Frame; // offset 0x1C, size 0x4
    float f32Speed; // offset 0x20, size 0x4
    float f32RandSpeed; // offset 0x24, size 0x4
    float f32FallSpeed; // offset 0x28, size 0x4
    float f32RandFallSpeed; // offset 0x2C, size 0x4
    signed int s32TexNo; // offset 0x30, size 0x4
    unsigned char u8r; // offset 0x34, size 0x1
    unsigned char u8g; // offset 0x35, size 0x1
    unsigned char u8b; // offset 0x36, size 0x1
    unsigned char u8a; // offset 0x37, size 0x1
    signed int s32RotX; // offset 0x38, size 0x4
    float f32SpeedInfl; // offset 0x3C, size 0x4
    class NNS_VECTOR sStartArea; // offset 0x40, size 0xC
    class NNS_VECTOR sScale; // offset 0x4C, size 0xC
    class NNS_VECTOR sRandScale; // offset 0x58, size 0xC
    class NNS_VECTOR sOfsVec; // offset 0x64, size 0xC
};
enum enmActionType {
    ACTION_TYPE_NULL = 0,
...
// Range: 0x3F0090 -> 0x3F0154
void draw(class clsWeather_Task * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3F0090 -> 0x3F0154
    }
...
// Range: 0x3F0160 -> 0x3F0910
void drawBill(class clsWeather_Task * this /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3F0160 -> 0x3F0910
        class stcDebugWeather * psInfo; // r20
        class clsPlayerTask * pcPlayer; // r2
        float (* paNnViewMtx)[4][4]; // r23
        unsigned char bDraw; // r22
        class stcSeed * psSeed; // r2
        class NNS_VECTORFAST sFollowPos; // r29+0x440
        unsigned int u32Col; // r2
        float sDebugMtx[4][4]; // r29+0x400
        float sRotMtx[4][4]; // r29+0x3C0
        class NNS_VECTOR sVec[38]; // r29+0x1F0
        class NNS_RGBA sCol; // r29+0x1E0
        signed int s32n; // r19
        signed int s32i; // r5
        float sDrawMtx[4][4]; // r29+0x1A0
        class NNS_VECTORFAST sPosVec; // r29+0x190
        float sMoveMtx[4][4]; // r29+0x150
        float sInvertMtx[4][4]; // r29+0x110
        class NNS_VECTORFAST asInFast[4]; // r29+0xD0
        class NNS_VECTORFAST asOutFast[4]; // r29+0x90
        signed int ts32PolygonNum; // @ 0x00635F50
        class NNS_PRIM3D_PCT tasPrim3D_PCT_Polygon[4]; // @ 0x00635EF0
    }
}


// Range: 0x3F0910 -> 0x3F1040
void drawNotBill(class clsWeather_Task * this /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3F0910 -> 0x3F1040
        class stcDebugWeather * psInfo; // r2
        class clsPlayerTask * pcPlayer; // r2
        unsigned char bDraw; // r22
        class stcSeed * psSeed; // r2
        class NNS_VECTORFAST sFollowPos; // r29+0x330
        unsigned int u32Col; // r2
        float sDebugMtx[4][4]; // r29+0x2F0
        float sRotMtx[4][4]; // r29+0x2B0
        class NNS_VECTOR sVec[38]; // r29+0xE0
        class NNS_RGBA sCol; // r29+0xD0
        signed int s32n; // r20
        float sDrawMtx[4][4]; // r29+0x90
        signed int ts32PolygonNum; // @ 0x00635ED0
        class NNS_PRIM3D_PCT tasPrim3D_PCT_Polygon[4]; // @ 0x00635E70
    }
}


// Range: 0x3F1040 -> 0x3F173C
void execute(class clsWeather_Task * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3F1040 -> 0x3F173C
        class stcDebugWeather * psInfo; // r2
        class clsPlayerTask * pcPlayer; // r19
        unsigned char bStop; // r17
        unsigned char bUsed; // r2
        float f32Speed; // r20
        class NNS_VECTORFAST sZVecFast; // r29+0xA0
        float sRotMtx[4][4]; // r29+0x60
        float f32Dist; // r29+0xB0
        unsigned char tbLastUsed; // @ 0x00B16D98
    }
}


// Range: 0x3F1740 -> 0x3F1D48
void setCreated(class stcStatus * psStatus /* r17 */, class stcSeed * psSeed /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3F1740 -> 0x3F1D48
    }
...
// Range: 0x3F1DA0 -> 0x3F2060
void setWeather(class clsWeather_Task * this /* r2 */, unsigned char u8Weather /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3F1DA0 -> 0x3F2060
    }
...
// Range: 0x3F2060 -> 0x3F3DA8
signed int DebugInfo(signed int s32xI /* r20 */, signed int s32yI /* r19 */, signed int s32ActiveNoI /* r22 */, signed int s32SpeedI /* r18 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3F2060 -> 0x3F3DA8
        signed int s32SelectMenuItem; // r21
        signed int s32yBack; // r23
        class stcDebugWeather * psInfo; // r17
        char * WindyName[8]; // r29+0x90
        char * PosName[2]; // r29+0xC8
        char * BlendName[2]; // r29+0xC0
        char * OnOffName[2]; // r29+0xB8
    }
}


// Range: 0x3F3DB0 -> 0x3F3FC4
void setDeafaultInfo(unsigned char u8Weather /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3F3DB0 -> 0x3F3FC4
        class stcStatus * opsStatus; // r2
    }
}


// Range: 0x3F3FD0 -> 0x3F413C
void * __ct(class clsWeather_Task * this /* r16 */, unsigned int u32View /* r18 */, unsigned char u8Weather /* r17 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3F3FD0 -> 0x3F413C
    }
