/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Effect\FollowLine\VaporTrail.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
class NNS_VECTOR StartPoint; // size: 0xC, address: 0x6860B0
class NNS_VECTOR AreaVec; // size: 0xC, address: 0x6860C0
static class stcBaseStatus oasBaseDataTbl[3]; // size: 0x24, address: 0x6860D0
static class stcPlayStatus oasPlayDataTbl[3]; // size: 0xB4, address: 0x686100
struct /* @anon0 */ {
    // total size: 0x30
} __vt__18clsVaporTrail_Task; // size: 0x30, address: 0x6C3380
...
// Range: 0x415130 -> 0x4155E4
void draw(class clsVaporTrail_Task * this /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x415130 -> 0x4155E4
        signed int s32MaxPlayNum; // r20
        unsigned char bDraw; // r22
        float (* paNnViewMtx)[4][4]; // r30
        float (& rsDrawMtx)[4][4]; // r29+0xA0
        signed int s32LineNum; // r19
        signed int s32n; // r18
        class stcParam * psParam; // r17
        class NNS_VECTOR sDefScale; // r29+0xD0
        class NNS_VECTOR sAddScale; // r29+0xC0
        unsigned char u8MaxAlpha; // r2
        signed int s32l; // r7
        unsigned char u8Alpha; // r15
        float f32Tu; // r11
        class NNS_VECTOR sScale; // r29+0xB0
        class NNS_PRIM3D_PCT tasPrimData[34]; // @ 0x00B16E30
    }
}


// Range: 0x4155F0 -> 0x4156D8
void execute(class clsVaporTrail_Task * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x4155F0 -> 0x4156D8
    }
...
// Range: 0x4156E0 -> 0x415F1C
void updateNormal(class clsVaporTrail_Task * this /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x4156E0 -> 0x415F1C
        float f32Speed; // r3
        unsigned char bUpdate; // r29+0xC0
        unsigned char bTrickAction; // r18
        unsigned char bGAction; // r17
        signed int s32PlayNum; // r30
        signed int s32n; // r22
        class stcParam * psParam; // r20
    }
}


// Range: 0x415FA0 -> 0x41628C
void * __ct(class clsVaporTrail_Task * this /* r18 */, unsigned int u32View /* r17 */, unsigned int u32PlayerNo /* r19 */, signed int s32DataNo /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x415FA0 -> 0x41628C
        unsigned int u32DataNo; // r16
        float f32AddMySpeed; // r21
        float f32AddEtcSpeed; // r20
        signed int s32n; // r4
    }
}
