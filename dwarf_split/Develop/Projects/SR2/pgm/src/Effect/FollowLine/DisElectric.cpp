/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Effect\FollowLine\DisElectric.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
class _NNS_TEXLIST * m_tpsTexList; // size: 0x4, address: 0xB17208
struct /* @anon0 */ {
    // total size: 0x30
} __vt__19clsDisElectric_Task; // size: 0x30, address: 0x6C46B0
struct /* @anon1 */ {
    // total size: 0x30
...
public:
    ...
    float f32Length; // offset 0x4, size 0x4
    float f32StTu; // offset 0x8, size 0x4
    signed int s32RotA32; // offset 0xC, size 0x4
};
enum enmFlag {
    FLAG_INFINI_CTRL = 1,
    FLAG_INFINI_DIVE = 2,
...
class clsDisElectric_Task : public clsTask {
    // total size: 0x10C
public:
    float m_f32TaFrame; // offset 0x54, size 0x4
    unsigned int m_u32ViewNo; // offset 0x58, size 0x4
    unsigned int m_u32PlayerNo; // offset 0x5C, size 0x4
    signed short m_s16Num; // offset 0x60, size 0x2
    float m_f32MaxLifeFrame; // offset 0x64, size 0x4
    float m_f32WaitFrame; // offset 0x68, size 0x4
    class stcParam m_asParam[10]; // offset 0x6C, size 0xA0
};
...
// Range: 0x43A2D0 -> 0x43A7E4
void draw(class clsDisElectric_Task * this /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x43A2D0 -> 0x43A7E4
        class clsPlayerTask * pcPlayer; // r2
        float sMtx[4][4]; // r29+0x150
        float (& rsDrawMtx)[4][4]; // r30
        class NNS_PRIM3D_PCT tasPrimData[6]; // r29+0xC0
        float f32HalfFrame; // r22
        signed int s32n; // r20
        class stcParam * psParam; // r2
        float f32AddLength; // r21
        float f32Height; // r20
        unsigned char u8MaxAlpha; // r2
        float tf32Scale; // @ 0x00636750
        float tf32w; // @ 0x00636740
        float tf32h; // @ 0x00636748
        float tf32Len; // @ 0x00636758
    }
}


// Range: 0x43A7F0 -> 0x43A9AC
void execute(class clsDisElectric_Task * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x43A7F0 -> 0x43A9AC
        signed int s32n; // r9
        class stcParam * psParam; // r8
        float tf32EfFrame; // @ 0x006366A0
    }
}


// Range: 0x43A9B0 -> 0x43ACC4
void * __ct(class clsDisElectric_Task * this /* r21 */, unsigned int u32View /* r17 */, unsigned int u32PlayerNo /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x43A9B0 -> 0x43ACC4
        signed int s32StartA32; // r2
        signed int s32Rot; // r2
        signed int s32n; // r20
        class stcParam * psParam; // r2
    }
}
