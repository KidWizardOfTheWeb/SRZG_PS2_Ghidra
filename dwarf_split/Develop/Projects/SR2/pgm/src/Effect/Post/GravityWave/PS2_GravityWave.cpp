/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Effect\Post\GravityWave\PS2_GravityWave.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
unsigned long long tau64GsPacket_DrawNega_NTSC[9][2]; // size: 0x90, address: 0x6276A0
unsigned long long tau64GsPacket_DrawNega_PAL[9][2]; // size: 0x90, address: 0x627730
unsigned long long tau64GsPacket_Capture_NTSC[11][2]; // size: 0xB0, address: 0x6277C0
unsigned long long tau64GsPacket_Capture_PAL[11][2]; // size: 0xB0, address: 0x627870
struct /* @anon2 */ {
    // total size: 0x40
...
class stcWaveVtxFmt {
    // total size: 0x30
public:
    class NNS_VECTORFAST sPosVF; // offset 0x0, size 0x10
    class NNS_VECTORFAST sNrmVF; // offset 0x10, size 0x10
    class NNS_VECTORFAST sUV_VF; // offset 0x20, size 0x10
};
...
public:
    ...
    unsigned char u8RegAddr; // offset 0x8, size 0x1
};
...
// Range: 0x35ACA0 -> 0x35AD64
void execute(class clsGravityWave_Task * this /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x35ACA0 -> 0x35AD64
        float f32FrameRateAdd; // r2
        float f32FrameRateAdd_Color; // r1
        class clsTaskManager & rcTaskMgr; // r2
        class clsBaseRace_Task & rcBaseRace_Task; // r2
        unsigned int u32ViewportNum; // r2
        unsigned int u32v; // r12
    }
}


// Range: 0x35AD70 -> 0x35AEDC
void drawOrthoModel(class clsGravityWave_Task * this /* r21 */, unsigned int u32v /* r20 */, class clsModelType_OB_TX_MO_MA_MF & rcModel /* r19 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x35AD70 -> 0x35AEDC
        unsigned int * psNodeStat; // r18
        float (* psMtxPal)[4][4]; // r17
        float aMtx[4][4]; // r29+0x70
    }
}


// Range: 0x35AEE0 -> 0x35B3C8
void replaceMorphUV(class clsGravityWave_Task * this /* r2 */, class NNS_OBJECT * psMorphObj /* r2 */, unsigned int u32v /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x35AEE0 -> 0x35B3C8
        class NNS_VTXLIST_PS2_VU_DESC_MORPH_OBJECT * psDesc; // r2
        class stcWaveVtxFmt * psVtx; // r8
        unsigned int u32MaxWidth; // r2
        unsigned int u32MaxHeight; // r2
        float af32MaxUV[4]; // r29+0x50
        float f32UOffset; // r1
        float f32VOffset; // r29+0x60
        float f32URate; // r5
        float f32VRate; // r4
        unsigned short au16VtxNums[26]; // r29+0x10
        unsigned int j; // r9
        unsigned int i; // r6
    }
}
...
// Range: 0x35B3D0 -> 0x35B544
void captureFullScreen(class clsGravityWave_Task * this /* r19 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x35B3D0 -> 0x35B544
        unsigned long long * pu64p; // r18
...
// Range: 0x35B550 -> 0x35BC68
void drawNega(class clsGravityWave_Task * this /* r17 */, unsigned int u32v /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x35B550 -> 0x35BC68
        ...
        class stcPrimitive sGravityPrimitive_NTSC[4]; // r29+0xB0
        class stcPrimitive sGravityPrimitive_PAL[4]; // r29+0x80
        class stcPrimitive * psGravityPrimitive[2]; // r29+0xD8
        unsigned long long * pu64p; // r21
        class stcPrimitive * psPrimitive; // r20
        class PXS_CONTEXTBUFFER * sCb; // r19
        unsigned int u32i; // r18
        signed int s32z; // r2
        unsigned int u32ColerSetTbl[4]; // r29+0x70
        unsigned int u32ColTbl; // r2
        unsigned int u32j; // r17
    }
}


// Range: 0x35BC70 -> 0x35BD08
void initModel(class clsModelType_OB_TX_MO_MA_MF & rcModel /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x35BC70 -> 0x35BD08
        class _NNS_TEXINFO & rsTexInfoList; // r2
        class PXS_TEXREG & rsTexReg; // r2
    }
}


// Range: 0x35BD10 -> 0x35BDEC
void * __ct(class clsGravityWave_Task * this /* r16 */, unsigned int u32PrioI /* r2 */, signed int s32FlgI /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x35BD10 -> 0x35BDEC
    }
