/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Object\Player\Ghost.cpp
...
class stcData oasData[]; // size: 0x0, address: 0x665360
signed int tas32CrcTbl[256]; // size: 0x400, address: 0xB176C0
struct /* @anon1 */ {
    // total size: 0x7C
} __vt__14clsPlayerGhost; // size: 0x7C, address: 0x6C5700
class clsGameMgr * m_tpcSingleton; // size: 0x4, address: 0x6E6980
class clsTaskManager * m_tpcSingleton; // size: 0x4, address: 0x6E69D0
struct /* @anon6 */ {
    // total size: 0x20
} __vt__14clsGravityDive; // size: 0x20, address: 0x6BB730
struct /* @anon7 */ {
    // total size: 0x20
} __vt__17clsGravityControl; // size: 0x20, address: 0x6BB710
struct /* @anon8 */ {
    // total size: 0x2C
...
static class NNS_VECTORFAST tosZVecFast; // size: 0x10, address: 0x67EE00
static class NNS_VECTORFAST tosYVecFast; // size: 0x10, address: 0x0
static class NNS_VECTORFAST tosXVecFast; // size: 0x10, address: 0x0
static class NNS_VECTORFAST tosZeroVecFast; // size: 0x10, address: 0x67EE10
...
class clsPlayerGhost : public clsPlayerTask {
    // total size: 0x1300
public:
    unsigned char m_bIsEnableExec; // offset 0x12F0, size 0x1
    unsigned char m_bStartFlag; // offset 0x12F1, size 0x1
};
class clsPlaySe_2D {
    // total size: 0x8
public:
...
// Range: 0x38BEF0 -> 0x38BF24
void setStartFlag(class clsGhost * this /* r2 */, unsigned char bStart /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x38BEF0 -> 0x38BF24
    }
...
// Range: 0x38BF40 -> 0x38C418
void execute(class clsGhost * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x38BF40 -> 0x38C418
        class clsBaseRace_Task * pcBaseRase; // r2
        class stcGhostSaveData & rcPlayData; // r19
        unsigned char bIsChange; // r18
        unsigned int u32TimeMSec; // r2
        class stcGhostData_1F & rcData; // r2
        float sPlayerDrawMtx[4][4]; // r29+0x80
        class NNS_VECTORFAST sPos; // r29+0x70
        class NNS_QUATERNION sQuat; // r29+0x60
        class NNS_VECTORFAST sVecTmp; // r29+0x50
        char __FUNCTION__[8]; // @ 0x0067EB28
    }
}


// Range: 0x38C420 -> 0x38D038
void updateGhostPlayer(class clsGhost * this /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x38C420 -> 0x38D038
        class stcGhostSaveData & rcPlayData; // r20
        class stcGhostDataBuf & rcPlayBuf; // r2
        class stcGhostData_1F & rcData; // r19
        unsigned char u8GhostEquipsGearFlags; // r2
        unsigned int u32DrawDataNo; // r18
        unsigned char u8BaseMotType; // r22
        unsigned char u8NewGearFlag; // r3
        unsigned char u8GearNo; // r2
        unsigned char bIsChangeMotion; // r2
        unsigned char u8NowMotNo; // r2
        float sMirrorMtx[4][4]; // r29+0x100
        class NNS_VECTORFAST sNextPos; // r29+0xF0
        class NNS_VECTORFAST sOldPos; // r29+0xE0
        class NNS_VECTORFAST sOld2Pos; // r29+0xD0
        class NNS_VECTORFAST & rsPos; // r17
        class NNS_QUATERNION & rsQuat; // r16
        float (& rsDrawMtx)[4][4]; // r22
        unsigned int au32DataNum[3]; // r29+0x150
        float af32PointMsec[3]; // r29+0x140
        class stcGhostData_1F * pocOldData; // r2
        class stcGhostData_1F * pocOld2Data; // r2
        unsigned int u32GhostFrameRate; // r2
        unsigned int u32Cnt; // r8
        float f321FrameMsec; // r23
        float f32NowMsec; // r20
        float f32Speed; // r22
        float f32CurveMotionRate; // r21
        class NNS_VECTORFAST sAddPos; // r29+0xC0
        class NNS_QUATERNION sOldQuat; // r29+0xB0
        float f32ScaleRate; // r20
        float f32NextSpeed; // r29+0x160
        char __FUNCTION__[18]; // @ 0x0067EB10
    }
}


// Range: 0x38D040 -> 0x38D090
void setGhostData(void * pvBuf /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x38D040 -> 0x38D090
    }
...
// Range: 0x38D090 -> 0x38D0D8
unsigned char checkGhostDataCrc(void * pvSaveData /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x38D090 -> 0x38D0D8
        class stcGhostSaveData * psSaveData; // r2
        unsigned int u32CrcData; // r29+0x2C
    }
}


// Range: 0x38D0E0 -> 0x38D120
void * getSaveDataBuff() {
    // Blocks
    /* anonymous block */ {
        // Range: 0x38D0E0 -> 0x38D120
    }
...
// Range: 0x38D120 -> 0x38D564
void restart(class clsGhost * this /* r18 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x38D120 -> 0x38D564
        unsigned char bIsCreatePlayer; // r16
        enum enmGearCtrl eGearCtrlNo; // r2
        char __FUNCTION__[21]; // @ 0x0067EAF0
        char __FUNCTION__[8]; // @ 0x0067EB08
        char __FUNCTION__[17]; // @ 0x0067EAD0
    }
}
