/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Object\Player\Key\ComSurvivalBallKey.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
unsigned char m_toaGoalLineObjNo[3]; // size: 0x3, address: 0x688AE8
struct /* @anon2 */ {
    // total size: 0xC8
...
unsigned int t_ou32ComGCtrlMinRestFrame; // size: 0x4, address: 0x693988
unsigned int t_ou32ComGCtrlMaxRestFrame; // size: 0x4, address: 0x693978
class clsPfSystem * m_tpcSingleton; // size: 0x4, address: 0x6E69B8
class NNS_VECTORFAST tosZVecFast; // size: 0x10, address: 0x688AF0
...
class _ComStartParam oasComStartParam[7]; // size: 0x38, address: 0x676E00
class clsGoalLine_Task * m_tpcSingleton; // size: 0x4, address: 0xB0A828
...
class _ComStartParam {
    // total size: 0x8
public:
    signed int s32DelayFrame60; // offset 0x0, size 0x4
    float f32Accel; // offset 0x4, size 0x4
};
enum enmDraw2DAttr {
...
class clsGoalLine_Task : public clsSingleModelGimmick_Task {
    // total size: 0x94
};
class DebugEntry {
    // total size: 0x3
...
class clsBaseGimmickBody : public clsBaseGimmickHead, public clsSingleton {
    // total size: 0x94
public:
    class clsGoalLine_Obj * m_pcObjArray; // offset 0x6C, size 0x4
    class vector m_vecGmkObjArray; // offset 0x70, size 0xC
    class vector m_vecUnusedGmkObjArray; // offset 0x7C, size 0xC
...
// Range: 0x4736D0 -> 0x4736D8
unsigned char controlGoalTime() {
    // Blocks
    /* anonymous block */ {
        // Range: 0x4736D0 -> 0x4736D8
    }
...
// Range: 0x4736E0 -> 0x473D80
unsigned int execGCtrlSlide(class clsComSurvivalBallKey * this /* r17 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x4736E0 -> 0x473D80
        float f32DifferenceX; // r29+0x9C
        float f32DifferenceY; // r29+0x98
        float f32RadX; // r20
        float f32RadY; // r29+0xA0
        signed char s8LeverX; // r2
        signed char s8LeverY; // r2
        unsigned int u32SetKeyState; // r16
        float sDrawMtx[4][4]; // r29+0x50
        class clsGravityControl * pcGravityControl; // r2
    }
}


// Range: 0x473D80 -> 0x4742D0
unsigned int execRun(class clsComSurvivalBallKey * this /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x473D80 -> 0x4742D0
        class NNS_VECTORFAST * psPlayerPos; // r20
        float f32Distance; // r29+0x150
        class NNS_VECTORFAST sPointRingPosVec; // r29+0x130
        class NNS_VECTORFAST sPointRingDirVec; // r29+0x120
        class NNS_VECTORFAST sPointRingDirVec2; // r29+0x110
        class clsSurvivalBallPointRing_Task * pcPointRingTask; // r2
        unsigned int u32PointRingNum; // r2
        float f32PointRingLength; // r20
        unsigned char bFlag; // r23
        signed int s32j; // r19
        float f32Length; // r29+0x150
        class NNS_VECTORFAST sDistDir; // r29+0x100
        float f32DotPlayer; // r20
        float f32DifferenceY; // r29+0x14C
        float f32Rad; // r29+0x150
        signed char s8LeverX; // r4
        unsigned int u32SetKeyState; // r5
    }
}


// Range: 0x4742D0 -> 0x474ABC
unsigned int execWalk(class clsComSurvivalBallKey * this /* r18 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x4742D0 -> 0x474ABC
        class clsItemBox_Task * pcItmBoxMgr; // r2
        unsigned int u32ItemNum; // r2
        unsigned int u32i; // r21
        class NNS_VECTORFAST * psPlayerPos; // r17
        class NNS_VECTORFAST * psDest; // r16
        float f32MinDistance; // r20
        float f32DistTmp; // r29+0x100
        class clsBaseRace_Task * pcRace; // r2
        float f32StartTime; // r29+0x100
        class clsGoalLine_Obj * pcGoalLine; // r16
        unsigned char u8ObjNo; // r2
        class NNS_VECTORFAST sFirstLineVec; // r29+0xE0
        class NNS_VECTORFAST sStartLineVec; // r29+0xD0
        class NNS_VECTORFAST sUpVec; // r29+0xC0
        class NNS_VECTORFAST sPlayer2StartDiff; // r29+0xB0
        float f32StartLineDirSp; // r29+0x100
        float f32Player2StartSp; // r29+0x100
        float f32DotTmpSp; // r29+0x100
        class _ComStartParam & rsStartParam; // r2
        float f32RestFrame60; // r29+0x100
        float f32DifferenceY; // r29+0xFC
        signed char s8LeverX; // r2
        signed char s8LeverY; // r2
        unsigned int u32SetKeyState; // r2
        float f32DotPlayer; // r20
        class NNS_VECTORFAST sDistDir; // r29+0xA0
    }
}


// Range: 0x474AC0 -> 0x474AC8
void @64@controlGoalTime__21clsComSurvivalBallKeyFSc() {
    // Blocks
    /* anonymous block */ {
        // Range: 0x474AC0 -> 0x474AC8
    }
...
// Range: 0x474AD0 -> 0x474AD8
void @64@__dt__21clsComSurvivalBallKeyFv() {
    // Blocks
    /* anonymous block */ {
        // Range: 0x474AD0 -> 0x474AD8
    }
