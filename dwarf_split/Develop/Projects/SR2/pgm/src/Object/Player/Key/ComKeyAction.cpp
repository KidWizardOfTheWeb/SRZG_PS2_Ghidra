/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Object\Player\Key\ComKeyAction.cpp
...
void t_pfExecAI[]; // size: 0x0, address: 0x6939D0
...
// Range: 0x32F590 -> 0x32F944
unsigned int execWheelDrift(class clsComKey * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x32F590 -> 0x32F944
        float f32DifferenceX; // r29+0x3C
        float f32DifferenceY; // r29+0x38
        float f32Rad; // r29+0x40
        signed char s8LeverX; // r2
        unsigned int u32SetKeyState; // r2
        float f32WheelDriftRestFrame; // r20
    }
}


// Range: 0x32F950 -> 0x32FA54
unsigned int execTrick(class clsComKey * this /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x32F950 -> 0x32FA54
...
// Range: 0x32FA60 -> 0x3300C0
unsigned int execGCtrlFlight(class clsComKey * this /* r17 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x32FA60 -> 0x3300C0
        float f32DifferenceY; // r29+0x7C
        float f32DifferenceX; // r29+0x78
        float f32RadX; // r29+0x80
        float f32RadY; // r29+0x80
        signed char s8LeverX; // r2
        signed char s8LeverY; // r2
        unsigned int u32SetKeyState; // r2
        float sDrawMtx[4][4]; // r29+0x30
        class clsGravityControl * pcGravityControl; // r2
    }
}


// Range: 0x3300C0 -> 0x3304B8
unsigned int execGCtrlSlide(class clsComKey * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3300C0 -> 0x3304B8
        float f32DifferenceY; // r29+0x7C
...
// Range: 0x3304C0 -> 0x330B74
unsigned int execGDive(class clsComKey * this /* r18 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3304C0 -> 0x330B74
        class clsBaseRace_Task * pcRace; // r2
        unsigned char bIsPathFound; // r2
        float f32DifferenceX; // r29+0x5C
        float f32DifferenceY; // r29+0x58
...
// Range: 0x330B80 -> 0x330C30
unsigned int execRail(class clsComKey * this /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x330B80 -> 0x330C30
    }
...
// Range: 0x330C30 -> 0x330CCC
unsigned int execJump(class clsComKey * this /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x330C30 -> 0x330CCC
    }
...
// Range: 0x330CD0 -> 0x331328
unsigned int execFly(class clsComKey * this /* r17 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x330CD0 -> 0x331328
        unsigned char bIsPathFound; // r2
        float f32DifferenceX; // r29+0x4C
        float f32DifferenceY; // r29+0x48
        float f32Rad; // r29+0x50
        signed char s8LeverX; // r2
        signed char s8LeverY; // r2
        unsigned int u32SetKeyState; // r2
    }
...
// Range: 0x331330 -> 0x3321A8
unsigned int execRun(class clsComKey * this /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x331330 -> 0x3321A8
        class clsBaseRace_Task * pcRace; // r20
        unsigned char bIsCorrection; // r29+0xD0
        unsigned int u32DisableRoot; // r2
        class NNS_VECTORFAST sPathDir; // r29+0x1B0
        class NNS_VECTORFAST sVirtualEndPoint; // r29+0x1A0
        class NNS_VECTORFAST sVirtualPathDir; // r29+0x190
        class NNS_VECTORFAST * psCalcEndPoint; // r18
        class NNS_VECTORFAST * psCalcPathDir; // r19
        class NNS_VECTORFAST sPlayerLinePos; // r29+0x180
        float f32Radius; // r20
        signed char s8Lever; // r2
        float f32DifferenceY; // r29+0x1CC
        float f32Rad; // r29+0x1D0
        signed char s8LeverX; // r2
        unsigned int u32SetKeyState; // r2
        class NNS_VECTORFAST sPlayerPos2EndPointDiff; // r29+0x170
        float f32Length; // r29+0x1D0
        float f32BlendRate; // r20
        class clsRetPath cRetComPathNext; // r29+0x150
        class NNS_VECTORFAST sVecTmp; // r29+0x140
        class NNS_VECTORFAST sEnd; // r29+0x130
        class NNS_VECTORFAST sVecTmp; // r29+0x120
        float f32DotPlayer; // r29+0x1D0
        float f32DotTmp; // r29+0x1D0
        class NNS_VECTORFAST sCrossPos; // r29+0x110
        class NNS_VECTORFAST sCrossDiff; // r29+0x100
        float f32CrossDistance; // r29+0x1D0
        class NNS_VECTORFAST sEndPoint2CrossPointDir; // r29+0xF0
        class NNS_VECTORFAST sEndPoint2PlayerDir; // r29+0xE0
        float f32DotNearSphere; // r29+0x1D0
        float f32DifferenceY; // r20
        float f32Rad; // r29+0x1D0
        float f32RestRad; // r29+0x1C8
        float f32AddPlayerRad; // r29+0x1D0
        class clsGearBasePrototype * pcGear; // r2
        unsigned int u32SetKeyState; // r2
        unsigned int u32SetKeyState; // r2
        enum enmKickerRankZone eTrickRank; // r2
        unsigned int u32TrickKey; // r17
        float f32RestFrame; // @ 0x00B0D970
    }
}


// Range: 0x3321C0 -> 0x332928
unsigned int execWalk(class clsComKey * this /* r18 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x3321C0 -> 0x332928
        class clsBaseRace_Task * pcRace; // r2
        ...
        float f32DifferenceY; // r29+0x9C
        signed char s8LeverX; // r2
        signed char s8LeverY; // r2
        unsigned int u32SetKeyState; // r2
        float f32DotPlayer; // r20
        class NNS_VECTORFAST sDistDir; // r29+0x80
        float f32RadAccele; // r29+0xA0
        float f32StartTime; // r29+0xA0
        class NNS_VECTORFAST sPlayer2StartDiff; // r29+0x70
        float f32StartLineDirSp; // r29+0xA0
        float f32Player2StartSp; // r29+0xA0
        float f32DotTmpSp; // r29+0xA0
        class _ComStartParam & rsStartParam; // r2
        float f32RestFrame60; // r29+0xA0
    }
}


// Range: 0x332930 -> 0x332938
unsigned int execDemo() {
    // Blocks
    /* anonymous block */ {
        // Range: 0x332930 -> 0x332938
    }
