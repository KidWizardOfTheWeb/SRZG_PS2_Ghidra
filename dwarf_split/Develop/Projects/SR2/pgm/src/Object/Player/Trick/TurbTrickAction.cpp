/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Object\Player\Trick\TurbTrickAction.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
struct /* @anon0 */ {
    // total size: 0x1C
} __vt__18clsTurbTrickAction; // size: 0x1C, address: 0x6CEB00
class clsCollisionManager * m_tpcSingleton; // size: 0x4, address: 0x703C18
class stcTask sTask; // size: 0x30, address: 0xB0A900
float of32ToLandMotionLinkFrame; // size: 0x4, address: 0x685D20
class clsPfSystem * m_tpcSingleton; // size: 0x4, address: 0x6E69B8
float of32LongMotionLinkFrame; // size: 0x4, address: 0x685450
float oaf32MotionStepUpFramePipe[]; // size: 0x0, address: 0x685D28
class stcTrickMotionTable2 osTrickMotionTablePipe; // size: 0x360, address: 0x685D40
class stcTrickMotionDataDir2 osTrickMotionPipeRankX; // size: 0x10, address: 0x6860A0
float tf32TrickDeg; // size: 0x4, address: 0x6868C8
float oaf32DistanceTable_Trick[][7]; // size: 0x0, address: 0x67B520
class stcBonusParam oasTurbBonusParam[][7]; // size: 0x0, address: 0x67C3F0
class stcKickerParam oasTurbParamTable[][7]; // size: 0x0, address: 0x67BD50
...
class stcTrickMotionRankTable2 {
    // total size: 0x90
public:
    class stcTrickMotionDataDir2 asMotionDataDir2[9]; // offset 0x0, size 0x90
};
class PXS_TEXREG {
    // total size: 0x20
public:
...
class stcTrickMotionDataDir2 {
    // total size: 0x10
public:
    class stcTrickMotionData asMotionData[2]; // offset 0x0, size 0x10
};
...
class stcTrickMotionData {
    // total size: 0x8
public:
    float f32MotionFrame; // offset 0x0, size 0x4
    signed short s16RotateDeg; // offset 0x4, size 0x2
    signed char s8RotDir; // offset 0x6, size 0x1
    signed char s8EffectFlag; // offset 0x7, size 0x1
};
class stcDetail {
    // total size: 0x14
public:
...
class stcTrickMotionTable2 {
    // total size: 0x360
public:
    class stcTrickMotionRankTable2 asMotionRankTable2[6]; // offset 0x0, size 0x360
};
class clsSingleton {
    // total size: 0x4
};
...
// Range: 0x53A050 -> 0x53A240
enum enmRank calcTrickRank(class clsPlayerTask * pcPlayer /* r18 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x53A050 -> 0x53A240
        ...
        float * pf32RankHeight; // r2
        float tf32RankHeight[2][3]; // @ 0x006538E0
    }
}


// Range: 0x53A240 -> 0x53A2DC
void settingTrickParam(class clsTurbTrickAction * this /* r2 */, class clsTrickParam & rcParam /* r16 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x53A240 -> 0x53A2DC
        enum enmRank eRank; // r2
    }
}


// Range: 0x53A2E0 -> 0x53A7B8
void execute(class clsTurbTrickAction * this /* r18 */, class clsTrickParam & rcParam /* r17 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x53A2E0 -> 0x53A7B8
        class NNS_VECTORFAST sMoveDirVecFast; // r29+0x60
        float f32PosturalRad; // r29+0x70
        float f32StepUpFrame; // r20
        float f32Rad; // r29+0x70
        class NNS_VECTORFAST sDirVecFast; // r29+0x50
    }
}


// Range: 0x53A7C0 -> 0x53B168
void init(class clsTrickParam & rcParam /* r21 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x53A7C0 -> 0x53B168
        float f32VelocityRad; // r21
        unsigned char u8IsAdjusted; // r22
        class stcKickerParam * opsParam; // r2
        class NNS_VECTORFAST sStartVF; // r29+0x130
        class NNS_VECTORFAST sEndVF; // r29+0x120
        class NNS_VECTORFAST sDirPosVF; // r29+0x110
        class NNS_VECTORFAST sCrossPosVF; // r29+0x100
        class NNS_VECTORFAST sDirVF; // r29+0xF0
        class NNS_VECTORFAST sUpVF; // r29+0xE0
        class NNS_VECTORFAST sToCPathVF; // r29+0xD0
        class NNS_QUATERNION sPosturalQuat; // r29+0xC0
        class NNS_VECTORFAST sNextPosVF; // r29+0xB0
        float f32NowRad; // r29+0x140
        float f32NewRad; // r20
        float f32Tmp; // r29+0x140
        signed char s8KeyDir; // r5
    }
}
