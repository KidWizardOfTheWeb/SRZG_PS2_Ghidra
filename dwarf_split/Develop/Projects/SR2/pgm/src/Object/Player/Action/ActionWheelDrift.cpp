/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Object\Player\Action\ActionWheelDrift.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
static enum enmMotion toeWheelDriftMotionTable[2][2]; // size: 0x10, address: 0x690340
...
class NNS_VECTORFAST tosZeroVecFast; // size: 0x10, address: 0x690370
...
class stcBurnOutEffect {
    // total size: 0x70
public:
    unsigned int u32Num; // offset 0x0, size 0x4
    unsigned int u32Size; // offset 0x4, size 0x4
    class NNS_VECTORFAST sBeginSp; // offset 0x10, size 0x10
    class NNS_VECTORFAST sAddSp; // offset 0x20, size 0x10
    class NNS_VECTOR sRandSp; // offset 0x30, size 0xC
    class NNS_VECTOR sLocalPos; // offset 0x3C, size 0xC
    class NNS_VECTOR sLocalPosRand; // offset 0x48, size 0xC
    float f32AirK; // offset 0x54, size 0x4
    float f32Mass; // offset 0x58, size 0x4
    float f32LifeFrame; // offset 0x5C, size 0x4
    float f32GroundK; // offset 0x60, size 0x4
    float f32ClipZ; // offset 0x64, size 0x4
};
enum enmComDis {
    COM_DIS_NON = 0,
    COM_DIS_20 = 1,
...
// Range: 0x4D2040 -> 0x4D3468
void actionWheelDrift(class clsPlayerTask * this /* r18 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x4D2040 -> 0x4D3468
        float f32OffsetRotate; // r20
        float f32Rad; // r29+0x80
        signed int s32MotionTableNo; // r2
        float f32SpeedRate; // r29+0x80
        float f32Speed; // r29+0x80
        float f32Speed; // r29+0x80
        float f32Rate; // r20
        float f32Speed; // r29+0x80
        float f32Rate; // r20
        float f32Speed; // r29+0x80
        float f32TimingRate; // r21
        float f32SpeedRate; // r1
        float f32Speed; // r1
        signed int s32MotionTableNo; // r2
        float f32MotionRate; // r1
    }
}


// Range: 0x4D3470 -> 0x4D3694
void actionGearWheelDrift_drawBurnOutEffect(class clsPlayerTask * this /* r20 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x4D3470 -> 0x4D3694
        class NNS_VECTORFAST sVeloFast; // r29+0x190
        class NNS_VECTOR sPos; // r29+0x1D0
        class stcBurnOutEffect oasDriftBurnOutStatus[2]; // r29+0xB0
        signed int s32n; // r19
        unsigned int BO_NUM; // r18
        unsigned int BO_SIZE; // r17
        class NNS_VECTORFAST BEGIN_SP; // r29+0xA0
        class NNS_VECTORFAST ADD_BEGIN_SP; // r29+0x90
        class NNS_VECTOR RAND_SP; // r29+0x1C0
        class NNS_VECTOR LOCAL_POS; // r29+0x1B0
        class NNS_VECTOR LOCAL_POS_R; // r29+0x1A0
        float AIR_K; // r24
        float MASS; // r23
        float LIFE_FRAME; // r22
        float GROUND_K; // r21
        float CLIP_Z; // r20
    }
}
...
// Range: 0x4D36B0 -> 0x4D38A0
void initWheelDrift(class clsPlayerTask * this /* r20 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x4D36B0 -> 0x4D38A0
