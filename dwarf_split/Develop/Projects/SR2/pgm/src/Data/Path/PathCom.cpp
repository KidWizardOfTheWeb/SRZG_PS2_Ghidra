/*
    Compile unit: C:\Develop\Projects\SR2\pgm\src\Data\Path\PathCom.cpp
...
class _ComUserData {
    // total size: 0x6
public:
    unsigned short u16GearChange; // offset 0x0, size 0x2
    unsigned short u16ComAttr; // offset 0x2, size 0x2
    unsigned char u8MVProb; // offset 0x4, size 0x1
    unsigned char u8ShortCutAttr; // offset 0x5, size 0x1
};
class clsPlaySe_2D {
    // total size: 0x8
public:
...
// Range: 0x32CAC0 -> 0x32CC8C
class NNS_VECTORFAST * searchTrickGCtrlConnectPoint(class clsPathCom * this /* r21 */, class _LineData * psStartLine /* r2 */, unsigned int u32Ability /* r20 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x32CAC0 -> 0x32CC8C
        class _LineData * psLine; // r19
        unsigned int u32TgtStartPointNo; // r2
        class _ComUserData * psComData; // r3
        class clsBaseRace_Task * pcRace; // r2
        char __FUNCTION__[29]; // @ 0x00675240
    }
}


// Range: 0x32CC90 -> 0x32CCEC
class NNS_VECTORFAST * searchGCtrlEndPoint(class clsPathCom * this /* r2 */, class _LineData * psStartLine /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x32CC90 -> 0x32CCEC
        class _BinData * psPathData; // r2
        class _LineData * psLine; // r2
        unsigned int u32TgtStartPointNo; // r2
    }
}


// Range: 0x32CCF0 -> 0x32D030
unsigned char searchCourseAIPath(class clsPathCom * this /* r19 */, class clsRetPath * pcRetIO /* r21 */, class NNS_VECTORFAST * psPosition /* r22 */, unsigned int u32DisableRoots /* r20 */, unsigned char bNextSearchFlag /* r29+0xBF */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x32CCF0 -> 0x32D030
        class _BinData * psPathData; // r2
        class _LineData * psLine; // r2
        class NNS_SPHERE sSphere1; // r29+0xF0
        class NNS_SPHERE sSphere2; // r29+0xE0
        class NNS_CAPSULE sCapsule; // r29+0xC0
        signed int s32LineNo; // r19
        unsigned char bIsFound; // r23
        float f32DistSq; // r20
    }
}


// Range: 0x32D030 -> 0x32D0B4
void serarchNextAILine(class clsPathCom * this /* r2 */, class clsRetPath * pcRetIO /* r2 */, signed int s32PointNo /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x32D030 -> 0x32D0B4
