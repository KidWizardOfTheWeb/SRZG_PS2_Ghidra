/*
    ...
    Language: C++
*/
float nngMatEnvMatrixExt[4][4]; // size: 0x40, address: 0x5F8E50
static float nnsUv0Matrix[4][4]; // size: 0x40, address: 0x5F8E90
static unsigned char cb_buf[256]; // size: 0x100, address: 0x6E10C0
...
// Range: 0x14B120 -> 0x14B788
signed int nnSetMaterialMultiTextureExtPS2(class NNS_MATERIAL_TEXMAP_DESC * pTexDesc1 /* r2 */, class NNS_MATERIAL_TEXMAP_DESC * pTexDesc2 /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x14B120 -> 0x14B788
        class NNS_MATERIAL_TEXMAP_DESC * pTexDesc[2]; // r29+0xC8
        class _NNS_TEXINFO * pTexInfoList; // r2
        class _NNS_TEXINFO * pTexInfo[2]; // r29+0xC0
        class sceGsTex1 GsTex1[2]; // r29+0xA0
        class sceGsTex0 GsTex0[2]; // r29+0x90
        unsigned int Flag; // r2
        class NVS_SVROBJ * pSvrObj[2]; // r29+0xB8
        unsigned int nTex; // r22
        unsigned int i; // r18
        signed int iTexIdx[2]; // r29+0xB0
        enum NNE_BOOL fPutTexture; // r17
        signed int ret; // r2
        unsigned int tfx; // r4
    }
}
...
// Range: 0x14B790 -> 0x14BB40
signed int nnSetMaterialSingleTextureExtPS2(class NNS_MATERIAL_TEXMAP_DESC * pTexDesc /* r18 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x14B790 -> 0x14BB40
        class _NNS_TEXINFO * pTexInfoList; // r2
        class _NNS_TEXINFO * pTexInfo; // r29+0x6C
        class NVS_SVROBJ * pSvrObj; // r17
        class sceGsTex1 GsTex1; // r29+0x60
        class sceGsTex0 GsTex0; // r29+0x58
...
// Range: 0x14BB40 -> 0x14BC00
void nnSetMaterialTextureWrapModeExtPS2(signed int context /* r2 */, class NNS_MATERIAL_TEXMAP_DESC * pTexDesc /* r2 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x14BB40 -> 0x14BC00
...
// Range: 0x14BC00 -> 0x14C5C4
enum NNE_BOOL nnPutMaterialCoreExt(class NNS_DRAWCALLBACK_VAL * val /* r20 */) {
    // Blocks
    /* anonymous block */ {
        // Range: 0x14BC00 -> 0x14C5C4
        class NNS_MATERIALPTR * mat; // r19
        class NNS_MATERIAL_SINGLE * pPs2mat; // r18
        class NNS_MATERIAL_MULTI * pPs2matMulti; // r19
        unsigned int fType; // r17
        unsigned int fGsPrimMode0; // r2
        unsigned int fGsPrimMode1; // r2
        unsigned int fTexMapType1; // r2
        class PXS_PRMODESET prmode; // r29+0x80
        class PXS_MATERIAL * pPxMat; // r2
        signed int ctx; // r17
        unsigned int fog_sw; // r2
        enum NNE_BOOL FixMaterial; // r5
        enum NNE_BOOL FixMaterial2; // r22
        enum NNE_BOOL bMod; // r16
        enum NNE_BOOL bDrawObjFlagDisableLighting; // r21
    }
}
